...
def main():
    ...
    for i in range(num_agentes):
        modelo_agente_1 = MLPClassifier(hidden_layer_sizes=(100, 50), max_iter=2000)
        modelo_agente_2 = KNeighborsClassifier()
        modelo_agente_3 = DecisionTreeClassifier()

        if i % 2 == 0:
            agente = Agente(f'A{i + 1}', modelo_agente_3, X_train, y_train, global_encoder)
        elif i % 1 == 0:
            agente = Agente(f'A{i + 1}', modelo_agente_2, X_train, y_train, global_encoder)
        else:
            agente = Agente(f'A{i + 1}', modelo_agente_1, X_train, y_train, global_encoder)
